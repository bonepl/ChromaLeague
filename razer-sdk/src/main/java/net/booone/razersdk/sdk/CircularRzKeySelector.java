...
public final class CircularRzKeySelector {
    ...
    private final List<Set<RzKey>> layers;
    ...
    private List<Set<RzKey>> buildLayers(RzKey startingKey) {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            Set<RzKey> newLayer = previousLayer.stream().flatMap(rzKey -> Stream.of(
                    RzKeyArray.keyAt(rzKey.getRow() - 1, rzKey.getColumn()),
                    RzKeyArray.keyAt(rzKey.getRow() - 1, rzKey.getColumn() + 1),
                    RzKeyArray.keyAt(rzKey.getRow() + 1, rzKey.getColumn()),
                    RzKeyArray.keyAt(rzKey.getRow() + 1, rzKey.getColumn() + 1),
                    RzKeyArray.keyAt(rzKey.getRow(), rzKey.getColumn() - 1),
                    RzKeyArray.keyAt(rzKey.getRow(), rzKey.getColumn() + 1)
