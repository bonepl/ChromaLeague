...
class AnimationTest {
    ...
    void testAddingFrameToFront() {
        ...
        final Map<RzKey2, Color> actualKeysToColors = animation.getFrame().getKeysToColors();
    ...
    void testAnimation() {
        ...
        animatedFrame.addAnimationFrame(new SimpleFrame(RzKey2.RZKEY_ENTER, Color.RED));
        animatedFrame.addAnimationFrame(new SimpleFrame(RzKey2.RZKEY_ENTER, Color.BLUE));
        ...
        //then
        assertTrue(animation.hasFrame());
        assertEquals(actualKeysToColors.get(RzKey2.RZKEY_ENTER), Color.RED);
        assertTrue(actualKeysToColors.entrySet().stream()
                .filter(entry -> entry.getKey() != RzKey2.RZKEY_ENTER)
                .map(Map.Entry::getValue).allMatch(color -> color == Color.GREEN));
        assertEquals(RzKey2.values().length, actualKeysToColors.size());

        //when 2nd frame
        final Map<RzKey2, Color> actualKeysToColors2 = animation.getFrame().getKeysToColors();

        //then
        assertFalse(animation.hasFrame());
        assertEquals(actualKeysToColors2.get(RzKey2.RZKEY_ENTER), Color.BLUE);
