...
public class HealthBar extends Animation {
    ...
    public Frame getFrame() {
        final double currentHp = RunningState.getGameState().getActivePlayer().championStats().getCurrentHealth();
        addToBack(getHpBar());
    ...
    public static List<RzKey> getHpBarPart(double previousHp, double currentHp) {
        final double maxHealth = RunningState.getGameState().getActivePlayer().championStats().getMaxHealth();
