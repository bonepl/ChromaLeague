...
import {GoldColor} from '../../../src/hud/colors/GoldColor.js';
...
/** Integration test for gold color animation on actual hardware. */
describe('Gold Color Integration', () => {
    test('displays gold color animation on keyboard', async () => {
        const sdk = new ChromaNativeSDK();
        await sdk.connect();
        try {
            const allKeys = getRzKeyValues();
            const keyColors = new Map(allKeys.map(key => [key, new GoldColor()]));
