...
import {AirColor} from '../../../src/hud/colors/AirColor.js';
...
/** Integration test for air color animation on actual hardware. */
describe('Air Color Integration', () => {
    test('displays air color animation on keyboard', async () => {
        const sdk = new ChromaNativeSDK();
        await sdk.connect();
        try {
            const allKeys = getRzKeyValues();
            const keyColors = new Map(allKeys.map(key => [key, new AirColor()]));
